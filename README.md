...
The server is the application orchestrator. The Ollama models are specialized services and do not communicate directly with each other.
...
├── main.py
├── models/
│   ├── api_models.py       # Typed API response models
│   ├── news_model.py       # News model alias
│   └── user_model.py       # User model alias
├── presenters/
│   ├── chat_presenter.py
│   ├── graph_presenter.py
│   ├── login_presenter.py
│   └── news_presenter.py
├── shared/
│   ├── api.py              # API operations, authentication, caching
│   ├── async_tasks.py      # Qt background workers
│   ├── cache.py            # TTL cache and request coalescing
│   └── http_client.py      # Thread-local HTTP connection pool
└── views/
    ├── chat_view.py
    ├── graph_view.py
    ├── login_view.py
...
The UI thread never performs network, database, or model calls directly. Requests run in Qt worker threads and return results through signals.
...
├── runtime.py              # Bounded executor for blocking integrations
...
├── controllers/            # HTTP routes and orchestration
├── models/                 # Pydantic API and domain models
├── queries/                # Read operations
├── commands/               # Write and refresh operations
├── gateway/
│   ├── newsapi_gateway.py
│   ├── ollama_client.py
│   └── ollama_gateway.py
...
FastAPI routes are asynchronous. Blocking SQL, NewsAPI, and Ollama operations run through a bounded executor so they do not block the event loop.

## News Flow

News is not fetched when the user navigates between pages.

```text
Application startup or "Load News"
    -> Client POST /news/refresh
    -> NewsAPI gateway
    -> Article validation
    -> Ollama summary generation
    -> SQL Server insert
    -> Client receives typed article models
    -> Client cache is updated
```

Regular `GET /news` reads stored articles only. This prevents unnecessary external requests.
